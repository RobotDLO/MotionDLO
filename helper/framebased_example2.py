#!/usr/bin/env python3
"""Frame-based cable polyline fitting, end to end.

Per-frame fitting with no tracking, so each frame is independent.

  1. SAM3Segmenter       binary masks from RGB frames
  2. FramePolyline       fit polylines to each mask
  3. FrameVisualization  render + save side-by-side vis
"""
...
OUTPUT_DIR = "path/to/output/Framebased"
...
import MotionDLO.helper.timing_utils as tu
...
# Timing comparison (kept identical to videobased_example.py)
WARMUP_FRAMES   = 1       # frames dropped from the per-frame statistic
EVENT_BUDGET_MS = 7.0     # event-branch update budget for the real-time reference line
...
def main():
    ...
    # warm up outside the timed loop so per-frame numbers are steady-state
    _warm = cv2.imread(frames[0])
    if _warm is not None:
        _t = time.perf_counter()
        segmenter.segment(_warm)
        print(f"[INFO] warmup/model-load (one-time, excluded): "
              f"{(time.perf_counter() - _t)*1000:.0f} ms")

    total_seg_time = 0.0
    total_fit_time = 0.0
    seg_times      = []          # per-frame SAM 3 image-segmentation times (s)
    ...
    for fi, frame_path in enumerate(frames, 1):
        ...
        # independent per-frame fit, no tracking state
        ...
        vis_polylines = polylines  # (id-less) for visualization
        ...
        # ── 3b. Accumulate for the consolidated polylines.npz ─────────────────
        if SAVE_NPZ:
            ...
            for Y in vis_polylines:
                ...
                if Yf.shape[0] != NUM_NODES:
                    # shouldn't happen (extract resamples), but pad/truncate so
                    # the stack below can't fail
    ...
    # ── Timing summary (per-frame image segmentation), matched to video ───────
    import torch
    gpu = torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu"
    stats = tu.summarize(seg_times, warmup=WARMUP_FRAMES,
                         label="SAM 3 per-frame segmentation")
    print(f"  TIMING  mode=frame  gpu={gpu}  seq={Path(FRAMES_DIR).parent}")
    print("  " + tu.fmt(stats))
    if stats.get("n", 0):
        print("  " + tu.budget_line(stats["mean_ms"], EVENT_BUDGET_MS))
    print(f"{'='*60}")
    tu.write_json(os.path.join(out, "timing_frame.json"), {
        "mode": "frame", "gpu": gpu, "sequence": str(FRAMES_DIR),
        "n_frames": len(frames),
        "event_budget_ms": EVENT_BUDGET_MS,
        "per_frame": stats,
    })
